...
Game = (function(superClass) {
  ...
  Game.prototype.onload = function(game) {
    var _, arrow, arrow_selected, assets, button, checkbox, cx, cy, i, j, k, l, menhir_texture, menhir_tile, mouse, random, scene, selected, selected_tile, sound, sounds, sprites, textures, tile, tiles, timeout, v, volume, x, y;
    assets = game.assets;
    ...
    selected = null;
    selected_tile = tiles.find(function(tile) {
      return Fz2D.distSqr(menhir_tile, tile) === 23120;
    });
    ...
    checkbox.onclick = function() {
      ...
      for (_ in sounds) {
        sound = sounds[_];
        results.push(sound.setVolume(volume).stop());
      }
      return results;
    };
    if (volume === 0) {
    ...
    button.onclick = function() {
      ...
      tiles.each(function(tile) {
        ...
        return tile.play('building');
      });
      return tiles.at(random.next(8) + (random.next(8) << 3)).play('menhir');
    };
    game.input.mouse.hide();
    mouse = scene.add(new Fz2D.Gui.Mouse(sprites.getTexture('cursor_hand')));
    return mouse.onclick = function() {
      var between_tile, xx, yy;
      ...
      if (selected != null) {
        ...
        if (tile.is('menhir') && (Math.abs(x) ^ Math.abs(y)) === 2) {
          xx = selected.xx + (x >> 1);
          yy = selected.yy + (y >> 1);
